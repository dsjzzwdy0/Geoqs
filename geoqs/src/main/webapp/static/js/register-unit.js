...
//加载layui的模块
layui.use(['form','layarea','multiSelect','upload'], function(){
...
function AcceptClick() {
	...
	$('select[multiple] option:selected').each(function() {
		deptTypes.push($(this).val());
	})
	if(deptTypes.length==0){
		dialogAlert("单位类型必须填写。");
		return false;
	}
    var user={
    		account:$("#account").val(),
    ...
    var dept = $("#form1").GetWebControls();
    var url=contentPath+"/doRegisterUnit";
    $.SaveForm({
        url: url,
        param: JSON.stringify({
        	dept:dept,
        	user:user,
        	deptTypes:deptTypes.join(',')
        }),
        contentType : 'application/json;charset=utf-8',
        loading: "正在保存数据...",
        success: function (data) {
        	dialogAlert("单位账号【"+user.account+"】注册成功，请等待审核通过。",1,function(){top.location.href=contentPath+"/login"});  
        }
    })
}
