...
$.fn.ComboBox = function (options) {
    //options参数：description,height,width,allowSearch,url,param,data,showFirst(增加默认显示第一条记录 xx)
    var $select = $(this);
    ...
    function loadComboBoxView(json, searchValue, m) {
        if (json.length > 0) {
            ...
            if (options.showFirst) {//增加默认显示第一条记录
                $select.ComboBoxSetValue(intiObj[options.id]);
            }
        }
        else {  //增加返回选项记录为0处理。
            var $_html = $('<ul></ul>');
            if (options.description != undefined) {
                ...
                $select.find(".ui-select-text").html("");
            }
            $option.find('.ui-select-option-content').html($_html);
...
$.fn.ComboBoxSetValue = function (value) {
    ...
    var $select = $(this);
    var $option = $("#" + $select.attr('id') + "-option");
    if ($option.find('ul').find('[data-value=' + value + ']').length == 0){
        return;
    }
    //==========================================================
...
dialogConfirm = function (content, callBack) {
    top.layer.confirm(content, {
        ...
        top.layer.close(index);//点击确定时  也关闭对话框
        callBack(true);
    }, function () {
        callBack(false)
    });
...
/*
 * type图标：0为感叹号，1为对勾，2为红×，3为问号，4为锁，5为不开心笑脸，6为开心笑脸
 * 
 */
    ...
    try {
        ...
        if (IsClose) {
            if (index) {
                ...
                top.window.close(); //20190226zzb暂时关闭，此处有严重bug，会造成系统关闭。体现在预算编制里上报功能。20190226,zl,可通过改变调用方式解决问题，解除注销
            }
        } else {
            location.reload();
        }
